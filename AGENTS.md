...
> 改协作流程请看 `AGENT.md`；改构建/基因/参数请看本文件。详见 `AGENT.md` §7.2。
>
> 仓库已于 2026-09-13 迁移：主工作树 = `the-world/` 根，远端 `gitee` = `the-world`；
> 实验数据在独立仓库 `the-world/the-world-data/`（**不进主仓**，见 `AGENT.md` §十一）。
