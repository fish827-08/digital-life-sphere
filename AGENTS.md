...
- 全量：~139 passed（双路径逐位对拍 + 快照 + 信号 + 愉悦度 + 基因注册表）
...
1. **性能线**：T1~T4 下沉全部并入（139 passed）；T5 slots 预研 + A3 敏感性扫描在云端（第二轮 C4/C5/C6）
2. **扩展特性优先级**：L9 尸体能量守恒 → L10 种子传播（云端 C5 进行中）→ L8 地形（非原 L10→L9→L8）
3. **审计线**：A1 清单 ✅ / A2 参数化 ✅ / A4 中性化建议 ✅；A3 敏感性表等云端 C4
4. **L7d Rayon**：暂缓；触发条件 = 单线程 60 万 tick > 2h 且 T1~T4 合入（当前 33 min，未到）
5. 评估材料 = 评估材料打包清单.md（12 项，9 项完成，A3 产出后补齐）
