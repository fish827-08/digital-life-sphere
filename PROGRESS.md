...
- 启动日期：2026-09-06
- 当前阶段：核心引擎（模块一~四）+ 性能线 T1~T4 下沉全部完成，特性线 L1~L6 落地 + L10a 先行版入库；30 万 tick L4+L5 长实验完成；A3 敏感性扫描由云端运行中；后续开发任务由云端开新分支承接，主干统一为 main（详见下方"已完成记录"与 AGENTS.md"当前阶段"）
...
| 2026-09-07 | **died字段bug修复** | _step_population返回的died只算了starved+expired，漏了predation；改成n_starved+n_expired+n_predation | 捕食死亡不计入died导致统计口径错误（deaths_by_cause有PREDATION但died=0），测试test_predation_kills发现此bug || 2026-09-08 | **L9 尸体能量守恒 + L8 地形（简化版）+ 季节系统** | 分支 feature/l9-terrain-season。L9：死亡能量×carcass_ratio(0.8)→尸体场，食腐消化率0.7，尸体分解率0.002/tick→50%转化资源，完整能量循环；L8地形：3种地形（平原/山地/水域），水域不可通行，山地移动能耗×2/资源再生×0.3，极地水域偏置；季节：黄赤交角0.4rad，太阳直射点sin摆动，year_length=3世界日（生物可体验1~3个四季），冬季半球资源/光合×0.3，渐进变化非突变。三大功能默认关闭，启用时自动回退Python路径。快照版本v2→v3（新增尸体场）。新增测试19例全通过。 | 设计文档：docs/L9-尸体-地形-季节设计文档.md。能量守恒方程验证通过。地形创造地理屏障→种群分化→方言形成的必要条件。季节迁移→跨纬度种群接触→文化交流。Rust下沉为后续工作（优先级：季节光照>尸体分解>地形移动） |
