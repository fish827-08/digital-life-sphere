...
class TestNeutralGenes:
    def test_genes_frozen_at_05(self):
        """neutral_genes=True 时，只冻结目标位 g14(感知)/g15(信号)，其余基因照常演化。

        C3 修正（2026-09-12）：原实现把所有基因钉在 0.5（连移动/代谢/繁殖也定型），
        会使零模型退化为"行为定型的假生态"；现只断开待检通路（g14/g15）。
        任务出处：`_share/待办与交接.md` C3。
        """
        ...
        assert p > 0
        genes = e._genes[:p]
        assert np.allclose(genes[:, 14], 0.5), "g14 感知应被冻结为 0.5"
        assert np.allclose(genes[:, 15], 0.5), "g15 信号应被冻结为 0.5"
        others = np.delete(genes, [14, 15], axis=1)
        assert not np.allclose(others, 0.5), (
            f"g0–g13 不应被冻结；实际范围 [{others.min()}, {others.max()}]"
        )
